...
COPY DeliverTableWorker/DeliverTableWorker.csproj                     DeliverTableWorker/
...
RUN dotnet restore DeliverTableWorker/DeliverTableWorker.csproj \
    -r linux-musl-x64

COPY DeliverTableWorker/          DeliverTableWorker/
COPY DeliverTableInfrastructure/  DeliverTableInfrastructure/
COPY DeliverTableSharedLibrary/   DeliverTableSharedLibrary/

RUN dotnet publish DeliverTableWorker/DeliverTableWorker.csproj \
    -c Release \
    -r linux-musl-x64 \
    --self-contained \
    -o /app/publish \
...
ENTRYPOINT ["/app/DeliverTableWorker"]
